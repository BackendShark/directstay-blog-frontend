      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 flex-1">
        {items.map((item) => (
          <Link
            key={item.id}
            href={`/blog/${item.slug}`}
            className="group block bg-white rounded-md overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
          >
            <div className="relative h-36 sm:h-40 lg:h-44 rounded-md overflow-hidden ">
              <Image
                src={item.image}
                height={200}
                width={300}
                alt={item.title}
                className="w-full rounded-md h-full object-cover transition-transform duration-500 ease-out group-hover:scale-110"
              />
              <div className="absolute rounded-md inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300"></div>
            </div>
